...
**Как обновить программу?** На macOS/Linux, если утилита запущена из
git-клона, она проверяет GitHub сама — при каждом запуске и по кнопке
**«Файл → Проверить обновления»** — и подтягивает изменения (`git pull
--ff-only`), затем перезапускается; если в рабочей копии есть свои
незакоммиченные правки, автообновление их не трогает и просто сообщает, что
обновление есть. На Windows программа собрана в exe и обновляется отдельно —
через мастер настройки (пункт «Contragenti — настройка и обновление» в меню
«Пуск») или переустановкой `setup.exe`/MSI.
