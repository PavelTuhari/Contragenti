...
| **Заказы** | вид **продажа / услуга / производство**, статусы (черновик → подтверждён → в работе → выполнен → оплачен / отменён), строки заказа с ценой из номенклатуры и итогом; кнопка **«Провести»**: продажа списывает остаток, производство приходует изделия, услуги остатки не трогают |
...
| **Главная** | показатели: клиенты, сумма открытых сделок, заказы за месяц, просроченные задачи, позиций номенклатуры; последние заказы, ближайшие задачи |
...
| `uCrmData.pas` | описания сущностей CRM (поля, типы, пресеты) и универсальный CRUD над SQLite; строки заказа, проводка остатков, конвертация лида |
