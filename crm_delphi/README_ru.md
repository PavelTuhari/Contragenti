...
| **Канбан** | доска работ с колонками-этапами и переносом карточки кнопками «← Назад» / «Вперёд →». Три доски: заказы (аванс → работа → отгрузка → оплата → закрыто), сделки по воронке, задачи по срокам. Перенос выставляет те же поля, по которым считаются этапы, поэтому доска и плитки рабочего стола всегда согласованы |
| **План работ (Гант)** | последовательность работ во времени: строка — заказ, под ним отступом его операции (строки заказа) в порядке исполнения. Светлая полоса — план, синяя — идёт, красная — выход за срок, зелёная — закрыт; вертикальная линия — сегодня. Фильтры: производство / все заказы / только незакрытые |
