...
- 🔀 **Три переключаемых источника**: портал **date.gov.md**, справочник
  **data2b.md** и **своя БД** (офлайн). Каждый включается и отключается
  отдельно, выбор **запоминается** между запусками. Портал отдаёт не все
  действующие компании (например IDNO `1007602003320`) — data2b.md закрывает
  эти пробелы мгновенно и без капчи.
...
| Источник | Как работает | Что даёт | Скорость |
|---|---|---|---|
| **date.gov.md** | Реальный Chrome (невидимая reCAPTCHA) | Полная карточка: форма, адрес, руководители, **учредители**, **задолженность** | 10–30 с |
| **data2b.md** | Обычный HTTP-запрос публичного поиска сайта | Название, IDNO, адрес | доли секунды |
| **своя БД** | Локальный SQLite | Всё, что накоплено ранее | мгновенно, офлайн |

Включённые источники опрашиваются **одновременно**; результаты объединяются и
дедуплицируются по IDNO, данные складываются в одну запись (адрес от data2b.md
дополняет карточку портала). Источник записи виден в карточке, попадает в
экспорт и в XML (`<counterparty source="…">`).

Переключатели **«Источники»** стоят рядом с полем поиска, их состояние
запоминается в `settings.json`. Если отключить оба онлайн-источника, поиск идёт
только по локальной базе. Ключ `--no-data2b` отключает data2b.md при запуске.

Когда портал отвечает «**Nu sunt date.**» (так он отвечает, например, на IDNO
`1007602003320`), утилита распознаёт это сразу и закрывает браузер, а не ждёт
таймаут — поиск возвращается за секунды вместо двух минут.
...
> **Scurt, în română:** [INSTALL_RO.md](INSTALL_RO.md) — instalare în cinci
> pași (setup.exe, asistent, prima căutare). Проверено на Windows 10/11 и
> Windows Server 2022.
...
программу в `C:\Program Files\Contragenti` (запрос UAC; без прав —
`/D=%LOCALAPPDATA%\Contragenti`), создаёт ярлыки и запись в «Программы и
компоненты», запускает мастер настройки. В установке уже есть базы с
данными: `companies.db` (компании date.gov.md) и `DemoCRM\clients.db`
...
запускает **мастер настройки** («ContragentiSetup.exe», он же в меню
...
| **[TECH_PASSPORT_ru.md](TECH_PASSPORT_ru.md)** | **Технический паспорт для развёртывания:** как поставить хаб, прослойку кассы и книгу на любом Linux-хостинге, в том числе по подпути существующего сайта — самодостаточная инструкция для ИИ-агента или инженера |
| **[Книга о комплексе](docs/book/index.html)** | **Одна страница про всё:** реестр, CRM на Delphi и Swift, сотрудники и ERP, хаб, касса Sunmi с фискализацией, установка и проверки — со схемой, снимками и выводом прогонов ([собрать заново](tools/make_book.py)) |
...
| **[IMPLEMENTATION_ru.md](IMPLEMENTATION_ru.md)** | **Руководство по внедрению:** варианты развёртывания, фазы, роли и RACI, процедуры, приёмочные испытания, ИБ, риски, показатели, формы актов |
| **[docs/metodichka.html](docs/metodichka.html)** | **Методичка со скриншотами:** 19 занятий от установки до приёмки — цель, шаги, снимок экрана, «должно получиться» и разбор типичных ошибок; чек-лист, контрольные вопросы, словарь |
