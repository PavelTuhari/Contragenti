...
- 🔌 **Локальный HTTP-API** (порт по умолчанию `9393`) + иконка в системном трее
...
Коротко: **весь код исполняется на компьютере пользователя, окно рисует
оконный менеджер ОС, данные лежат на локальном диске, серверной части нет.**
Подробное объяснение с разбором частых возражений — в
