...
`0001…0018` (1–3 строки, всего 36) + по одному производственному `PR-10xx`
на проект; 24 задачи (первые 4 выполнены) + задачи проектов из 11 шагов
`PROJECT_STEPS` (шаг «аванс» пропускается при `prepay_pct = 0`; проигранные
тендеры — только 2 шага); 10 проектов. Авансы/оплаты/отгрузки
...
4. **Demo CRM в Xcode — данные и CLI.** `xcodebuild … build` без ошибок;
   ...
   --seed-demo` на Windows (сверить `SELECT count(*)` по всем таблицам с
   `crm_delphi/seed.log`); `--dml-test` → 0; база, засеянная Delphi
